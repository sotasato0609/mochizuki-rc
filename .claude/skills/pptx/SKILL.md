...
**提案資料は骨子（`docs/*-outline.md`）をベースに 10枚前後で作る。** 詳細な分析は Markdown レポート側に置き、
スライドには結論と根拠の要点だけを載せる。

> 一度、分析レポートをそのままスライド化して27枚になり「長すぎる」と差し戻された（2026-07-23）。
> **分析の厚み＝スライド枚数ではない。** 厚みは Markdown レポートで担保し、スライドは意思決定に必要な枚数に絞る。

目安の構成（10枚）: 表紙／サマリー／現状／分析の要点×3／課題と打ち手／プラン／KPIと進行／依頼事項。
セクション区切り（Divider）は10枚程度なら不要（参照資料も15枚でDividerなし）。
